...
def _parse_with_pymupdf(pdf_path: str) -> List[ClauseChunk]:
    """Parse a text-based PDF using PyMuPDF."""
    try:
        import fitz  # PyMuPDF
    except ImportError:
        raise ImportError(
...
def parse_and_chunk(
    ...
    if use_azure_ocr:
        chunks = _parse_with_azure_ocr(pdf_path)
    else:
        chunks = _parse_with_pymupdf(pdf_path)

    print(f"[parse_chunk] '{os.path.basename(pdf_path)}' → {len(chunks)} clause chunks")
    return chunks
